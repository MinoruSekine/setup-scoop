...
if (($env:CACHE_HIT -ne 'true' -and $env:INSTALL_SCOOP -eq 'true') `
    -or $env:INSTALL_SCOOP -eq 'force') {
    $params = @()
...
if ($env:UPDATE_PATH -eq 'true') {
    $scoopDirPath = if ($env:SCOOP) { $env:SCOOP } else { "$env:USERPROFILE/scoop" }
    $scoopShimsPath = Join-Path -Path $scoopDirPath -ChildPath "shims"
    $scoopShimsPath >> $Env:GITHUB_PATH
...
# Clear scoop download cache before create cache of Scoop directory.
